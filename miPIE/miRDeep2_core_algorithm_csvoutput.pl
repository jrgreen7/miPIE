...
##SMIRP
# All SMIRP features contained in a hash, then output
my %SMIRP_features;
#matureBPRatio: fraction of mature nts that are bps
#lowerStemPairs: number of pairs in lower stem (not part of mature)
#scoreMFE: Score of MFE of pre-miRNA (based on log-odds...)
#priMFE: MFE of pre-miRNA
#pr_mature: number of reads mapping to mature miRNA
#pr_loop: number of reads mapping to loop region
#pr_star: number of reads mapping to star region
#pr_total: total number of reads mapping to any of these regions
#pr_inconsistent: total number of reads that do not map to any valid region
#totalReads: Total read count of consistent and inconsistent
...
#    this is the actual classification
    unless(pass_filtering_initial() and pass_threshold_score())
	{
		$ret=0;
	}
...
sub print_SMIRP
{
    #prints the 'SMIRP' hash of interesting features
    my @keys=sort {$a cmp $b} keys %SMIRP_features;
	
	# Print the SMIRP header only once
	if($SMIRP_printHeader)
	{
		print SMIRP_OUT join(',',@keys),"\n";
		$SMIRP_printHeader = 0;
	}
	
	my @vals;
    foreach my $key(@keys){
		push(@vals,$SMIRP_features{$key});
    }
	print SMIRP_OUT join(',',@vals),"\n";
}
...
#    this is the scoring
#############################################################################################################################################################################
# score_mfe is the well-behaved log-odds MFE score
#############################################################################################################################################################################
    #minimum free energy of the potential precursor
    my $score_mfe=score_mfe($hash_comp{"pri_mfe"});

	## SMIRP Output this feature for SMIRP
	$SMIRP_features{'scoreMFE'} = $score_mfe;
	$SMIRP_features{'priMFE'} = $hash_comp{"pri_mfe"};
	

#############################################################################################################################################################################
# hash_comp(freq_total) has the number of reads consistent with dicer processing (possible feature)
#############################################################################################################################################################################
...
#############################################################################################################################################################################
# Can grab this while we're at it - untested feature
# Appears to give a score depending on how many of the +/-10bp appended to the 
#  mature that form bp's. More is better. Score captured in array "scores_stem"
#############################################################################################################################################################################
    if($options{z}){
		...
		## SMIRP Output this feature for SMIRP
		$SMIRP_features{'lowerStemPairs'} = $stem_bp;
    }


    #score for randfold (optional)
    if($options{y}){
		#IF randfold p-value not defined for this potential precursor
		if(not(defined $hash_randfold{$hash_comp{"pri_id"}}))
		{
			$hash_comp{"score_randfold"}="0";
		}

#############################################################################################################################################################################
# score_randfold itself seems like a better feature than thresholding
#############################################################################################################################################################################
...
#############################################################################################################################################################################
# Extract star_perfect / (star_perfect + star_fuzzy)? How to deal with case of no star reads?
#############################################################################################################################################################################
...
#    score for the presence of star evidence
	...
	##SMIRP: Grab these read counts for SMIRP features
	my $allReadsCount = $consistent+$inconsistent;
	$SMIRP_features{"totalReads"}= $allReadsCount;
	$SMIRP_features{"pr_mature"}= $freq_mature / $allReadsCount;
    $SMIRP_features{"pr_loop"}= $freq_loop / $allReadsCount;
    $SMIRP_features{"pr_star"}= $freq_star / $allReadsCount;
    $SMIRP_features{"pr_total"}= $consistent / $allReadsCount;
    $SMIRP_features{"pr_inconsistent"}= $inconsistent / $allReadsCount;
	
    unless($consistent+$inconsistent>0){$hash_comp{"problem_read_cnt"}="no reads map to the candidate\n"; return 0;}

#############################################################################################################################################################################
# inconsistent_fraction is another possible feature
#############################################################################################################################################################################
...
sub bp_duplex{
	...
	##SMIRP Grab this ratio (fraction of mature nts that are bps
	$SMIRP_features{'matureBPRatio'} = $duplex_bps/$lng;
...
#############################################################################################################################################################################
# Calculation of log odds MFE score
#############################################################################################################################################################################
sub score_mfe{
